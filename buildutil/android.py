...
# NDK location storage file.
_NDK_LOCATION = '.ndk_install_path'
...
class BuildEnvironment(common.BuildEnvironment):
  ...
  @staticmethod
  def ndk_location_changed(project, ndk_build_path):
    ...
    Returns:
      True if the NDK was previously used to build the project and the
      NDK install location changed.
    """
    current_ndk_path = os.path.dirname(ndk_build_path)
    old_ndk_path = ''
    ndk_location_filename = os.path.join(project, _NDK_LOCATION)
    if os.path.exists(ndk_location_filename):
      with open(ndk_location_filename) as f:
        old_ndk_path = f.read()

    return current_ndk_path != old_ndk_path

  @staticmethod
  def write_ndk_location(project, ndk_build_path):
    ...
    current_ndk_path = os.path.dirname(ndk_build_path)
    ndk_location_filename = os.path.join(project, _NDK_LOCATION)
    project_dir = os.path.dirname(ndk_location_filename)
  ...
  def build_android_libraries(self, subprojects, output=None,
    ...
    Args:
      ...
      check_ndk_install_path: Whether to track the NDK install location and
    ...
    for p in subprojects:
      ...
      if check_ndk_install_path:
        project_output_dir = (self.get_project_directory(path=output)
                              if output else p)
        if BuildEnvironment.ndk_location_changed(project_output_dir,
                                                 ndk_build):
          # If the NDK location changes, the clean target will not run to
          # completion.  Instead manually delete all files generated by the NDK
          # build system, and store the NDK location file for future builds.
          output_directories = (
            [project_output_dir] if project_output_dir else
            [os.path.join(p, 'obj'), os.path.join(p, 'libs')])
          for output_dir in output_directories:
            if os.path.exists(output_dir):
              shutil.rmtree(output_dir)
          BuildEnvironment.write_ndk_location(project_output_dir, ndk_build)
