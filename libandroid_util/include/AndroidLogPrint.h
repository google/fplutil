...
#define fflush(s) AndroidPrintfFlush(s)
#define perror(s) AndroidLogPerror(s)
#else
#define printf(...) \
...
#define ANDROID_LOG_ERROR(...) \
  (__android_log_print(ANDROID_LOG_ERROR, gAndroidLogPrintfTag, __VA_ARGS__))
#define ANDROID_LOG_INFO(...) \
  (__android_log_print(ANDROID_LOG_INFO, gAndroidLogPrintfTag, __VA_ARGS__))
...
// perror.  Used by AndroidLogPerror() and factored out to be testable.
//
// Arguments:
// msgin - An optional message to prepend to the error to be printed.  May
//         be NULL.
...
//
// sprintf(msgout, "%s: %s", msgin, strerror(err));
// or
// strcpy(msgout, strerror(err));
//
// depending on the value of msgin.
void AndroidPerrorMsg(const char *msgin, int err, char *msgout, size_t outsize);

// A perror replacement that prints to the android log. See the perror man
// page at:
...
// Wrapper for fflush.
int AndroidPrintfFlush(FILE *stream);
